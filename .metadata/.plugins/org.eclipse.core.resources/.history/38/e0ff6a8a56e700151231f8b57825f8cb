	...
	public void loadMessages(Statement stmt) {
		try {
			...
			while(rs1.next()) {
				...
				Message toAdd = new Message(type, username, recipient, message, subject, ID, null, null, -1);
			...
			while(rs2.next()) {
				...
				Message toAdd = new Message(type, sender, username, message, subject, ID, read, null, -1);
